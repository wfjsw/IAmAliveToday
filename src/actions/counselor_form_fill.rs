...
pub fn get_form_list(session: &Client, base_url: &str, page_size: u32, page_number: u32) -> Result<Vec<CollectorFormInstance>> {
    let result = session.post(format!("{}/wec-counselor-collector-apps/stu/collector/queryCollectorProcessingList", base_url))
        .json(&json!({
            "pageSize": page_size,
    ...
    match result.status() {
        StatusCode::OK => {
            let resp : CounselorResponse<CounselorPaginator<CollectorFormInstance>> = result.json()?;
            let data = resp.datas;
...
mod tests {
    ...
    use crate::actions::counselor_form_fill::{CollectorFormInstance, CounselorPaginator, CounselorResponse};
    ...
    fn test_counselor_form_list_deserialise() {
        ...
        let parsed_response : CounselorResponse<CounselorPaginator<CollectorFormInstance>> = serde_json::from_str(response).unwrap();
        ...
        assert_eq!(parsed_response.datas.rows[0].content, "https://wecres.cpdaily.com/counselor/test/html/test.html");
        assert_eq!(parsed_response.datas.rows[0].sender_user_name, "test(test)");
        assert_eq!(parsed_response.datas.rows[0].create_time, "2021-11-08 00:16");
        ...
        assert_eq!(parsed_response.datas.rows[0].current_time, "2021-11-08 14:25:38");
    ...
    fn test_get_form_list() {
        ...
        headers.append(COOKIE, HeaderValue::from_str(&format!("MOD_AUTH_CAS={}", &mod_auth_cas)).unwrap());
